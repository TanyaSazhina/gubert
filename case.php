...
$case_arr = mysqli_query($conn, "SELECT * FROM `cases` WHERE `id` =" . $id);
$imgs = mysqli_query($conn, "SELECT * FROM `cases_imgs` WHERE `id_case` = " . $id . " ORDER BY `serial_number` ASC");
$case = mysqli_fetch_assoc($case_arr);
$next_id = $case['id'] + 1;
$prev_id = $case['id'] - 1;
$next = mysqli_query($conn, "SELECT * FROM `cases` WHERE `id` =" . $next_id);
if ($next) {
    $next = 1;
} else {
    $next = 0;
};
$prev = mysqli_query($conn, "SELECT * FROM `cases` WHERE `id` =" . $prev_id);
if ($prev) {
    $prev = 1;
} else {
    $prev = 0;
...
<body>
    ...
    <main class="page">
        <section class="case">
            <div class="case__discription container">
                <h1 class="case__title title">
                    <?= $case['name'] ?>
                </h1>
                <p class="case__text">
                    <b>Задача:</b> <?= $case['description'] ?>
                </p>
            </div>
            <div class="case__screen gray">
                <div class="case__container container">
                    <h2 class="case__screen-title title">
                        Экраны
                    </h2>
                    <?php foreach ($imgs as $img) { ?>
                        <img class="case__img" src="img/cases/screens/<?= $case['name'] ?>/<?= $img['link'] ?>" alt="screen">
                    <?php } ?>
                </div>
            </div>
        </section>
        <section class="case__nav container">
            <?php
            if ($prev) { ?>
                <a href="case.php?id=<?= $prev_id ?>">⟵ имя прошлого проекта</a>
            <?php }
            if ($next) { ?>
                <a href="case.php?id=<?= $next_id ?>">имя следующего проекта ⟶</a>
            <?php } ?>
        </section>
    </main>
